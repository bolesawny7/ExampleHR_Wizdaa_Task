...
describe('BalancesService', () => {
  ...
  test('throws BalanceNotFoundError for unknown key', () => {
    expect(() => svc.getEffectiveBalance('E-x', 'L-x', 'ANNUAL'))
      .toThrow(/No balance for key/);
  });
  ...
  test('applyHcmBalanceSnapshot ignores stale asOf', () => {
    svc.applyHcmBalanceSnapshot({
      employeeId: 'E-1', locationId: 'L-1', leaveType: 'ANNUAL',
      balance: 10, asOf: 2000,
    });
    const r = svc.applyHcmBalanceSnapshot({
      employeeId: 'E-1', locationId: 'L-1', leaveType: 'ANNUAL',
      balance: 20, asOf: 1000,
    });
  ...
  test('listing returns all (location, leaveType) rows for employee', () => {
    seed(10);
    svc.applyHcmBalanceSnapshot({
      employeeId: 'E-1', locationId: 'L-2', leaveType: 'ANNUAL',
      balance: 5, asOf: clock.now(),
    });
    svc.applyHcmBalanceSnapshot({
      employeeId: 'E-1', locationId: 'L-1', leaveType: 'SICK',
      balance: 7, asOf: clock.now(),
    });
