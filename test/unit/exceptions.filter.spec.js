...
function makeHost(capturedStatus = {}, capturedBody = {}) {
  return {
    switchToHttp: () => ({
      getResponse: () => ({
        status(code) { capturedStatus.code = code; return this; },
        json(payload) { Object.assign(capturedBody, payload); return this; },
      }),
    }),
  };
}
...
describe('AllExceptionsFilter', () => {
  ...
  test('maps an HttpException to its native status and body object', () => {
    const status = {}, body = {};
    filter.catch(new HttpException({ error: 'BOOM', message: 'boom' }, 418), makeHost(status, body));
