...
describe('TimeOffService', () => {
  ...
  test('cancel by non-owner is forbidden', () => {
    ...
    expect(() => svc.cancel({ actor: other, requestId: r.id })).toThrow(
      /Cannot cancel another employee/,
    );
  });
  ...
  test('manager cannot approve requests of employee outside their team', () => {
    ...
    expect(() => svc.approve({ actor: otherManager, requestId: r.id })).toThrow(
      /Not this employee/,
    );
  });
  ...
  test('double-approve is idempotent at second call (invalid transition)', () => {
    ...
    svc.approve({ actor: manager, requestId: r.id });
    expect(() => svc.approve({ actor: manager, requestId: r.id })).toThrow(
  ...
  test('approve surfaces ConflictError if the row is raced to a different state', () => {
    ...
    expect(() => svc.approve({ actor: manager, requestId: r.id })).toThrow(
      /CONCURRENT_UPDATE|concurrently/,
    );
  });
